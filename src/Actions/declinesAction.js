...
const getDeclines = (declines) => ({
    type : types.GET_DECLINES,
    payload : declines,
});

const declineAdded = () => ({ 
    type : types.ADD_DECLINE,
});

const declineDeleted = () => ({
    type : types.DELETE_DECLINE,
})

const getDecline = (decline) => ({
    type : types.GET_SINGLE_DECLINE,
    payload: decline,
})

const declineUpdated = () => ({
    type : types.UPDATE_DECLINE,
});

// get all categoryes
export const loadDeclines = () => {
    return function (dispatch) {
        axios.get(`${process.env.REACT_APP_IPURL}/api/admin/decline-product`)
        .then((res) => {
            // console.log("reponse categoryes:", res);
            dispatch(getDeclines(res.data))
        })
        .catch((error) => {
            // console.log(error);
        })
