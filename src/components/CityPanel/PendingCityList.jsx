...
const PendingCityList = () => {
    ...
    const displayUsers = pendingCitys.filter((data) => {
        if (searchVal === "") {
            return data;
        } else if (
        ...
        return (
            <tr>
                ...
                {/* <td>
                    <img src={`${process.env.REACT_APP_IPURL}${state.city_icon}`} alt="ads" />
                </td> */}
                <td>
    ...
    finder = pendingCitys.filter((val) => {
        if (searchVal === "") {
            return val;
        } else if (
    ...
    const pageCount = Math.ceil(pendingCitys.length / usersPerPage);
    const changePage = ({ selected }) => {
        setPageNumber(selected)
    }
    // DOWNLOAD IN EXCEL
    const fileName = 'pending-city-Excel'
    return (
        <div className="container">
            <div className="col-lg-12 grid-margin stretch-card">
                <div className="card">
                    <div className="card-body">
                        <h4 className="card-title">Pending City List <small>Dashboard</small></h4>
                        {/* <Link
                            class="action-button badge badge-primary"
                            to="/City/Create">Add City</Link> */}
                        {/* <button className="action-button badge badge-primary">
                            <FontAwesomeIcon icon="download" />
                            <ExportCSV csvData={pendingCitys} fileName={fileName} />
                        </button> */}
...
export default PendingCityList